function solve() {
  ...
  const actions = {
    "Quick Check": function (e) {
      const inputElements = [...tableEl.querySelectorAll("tbody td input")].map(
        (el) => +el.value
      );
      ...
      for (let i = 1; i <= lengthRowAndColumn; i++) {
        sudokuNumber += i;
      }
      ...
      for (let row = 0; row < lengthRowAndColumn; row++) {
        ...
        if (
          isSolved &&
          (rowAmount !== sudokuNumber || columnAmount !== sudokuNumber)
        )
          isSolved = false;

        leftDiag +=
          inputElements[
            lengthRowAndColumn * row + lengthRowAndColumn - 1 - row
          ];
      ...
      if (
        ...
        ((leftDiag === sudokuNumber &&
          rightDiag !== lengthRowAndColumn &&
          rightDiag !== Math.pow(lengthRowAndColumn, 2)) ||
          (rightDiag === sudokuNumber &&
            leftDiag !== lengthRowAndColumn &&
            leftDiag !== Math.pow(lengthRowAndColumn, 2)))
      ) {
        isSolved = false;
      }
    ...
    changeStyle: (el, styleProp, value) => {
      el.style[`${styleProp}`] = value;
    },
    changeContent: (el, contentProp, value) => {
      el[`${contentProp}`] = value;
    },
  };
